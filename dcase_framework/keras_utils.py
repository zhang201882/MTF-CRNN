    ...
    def _setup_keras(self):
        ...
        if not hasattr(self, 'keras_setup_done') or not self.keras_setup_done:
            ...
            if self.show_extra_debug:
                ...
                self.logger.debug('    Backend \t[{backend}]'.format(backend=self.learner_params.get_path('keras.backend', 'theano')))
            ...
            if self.learner_params.get_path('keras.backend', 'theano') == 'theano':
                ...
                if self.learner_params.get_path('keras.backend_parameters.device'):
                    ...
                    if self.show_extra_debug:
                        self.logger.debug('    Device \t\t[{device}]'.format(device=self.learner_params.get_path('keras.backend_parameters.device', 'cpu')))
                ...
                if self.learner_params.get_path('keras.backend_parameters.floatX'):
                    ...
                    if self.show_extra_debug:
                        self.logger.debug('    floatX \t\t[{float}]'.format(float=self.learner_params.get_path('keras.backend_parameters.floatX', 'float32')))
                ...
                if self.show_extra_debug:
                    self.logger.debug('    Optimizer \t[{optimizer}]'.format(optimizer=self.learner_params.get_path('keras.backend_parameters.optimizer', 'None')))
                ...
                if self.learner_params.get_path('keras.backend_parameters.fastmath') and self.learner_params.get_path(
                    ...
                    if self.show_extra_debug:
                        self.logger.debug('    NVCC fastmath \t[{flag}]'.format(flag=str(self.learner_params.get_path('keras.backend_parameters.fastmath', False))))
                ...
                message = '{name}: Keras backend not supported [backend].'.format(name=self.__class__.__name__, backend=self.learner_params.get_path('keras.backend'))
                self.logger.exception(message)
                raise AssertionError(message)
            if self.show_extra_debug:
                self.logger.debug('  ')
