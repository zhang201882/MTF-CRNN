...
class BaseRecognizer(object):
    ...
    def process_activity(self, activity_array, window_length, operator="median_filtering"):
        ...
        if operator == 'median_filtering':
            ...
            message = '{name}: Unknown activity processing type [{type}].'.format(name=self.__class__.__name__, type=operator)
            self.logger.exception(message)
            raise AssertionError(message)
...
class EventRecognizer(BaseRecognizer):
    ...
    def process(self, frame_probabilities):
        ...
        if isinstance(frame_probabilities, tuple) and len(frame_probabilities) == 2:
            ...
            return self.process_matrix(frame_probabilities=frame_probabilities)
        ...
        for event_id, event_label in enumerate(self.class_labels):
            ...
            if self.params.get_path('frame_binarization.enable'):
                if self.params.get_path('frame_binarization.type') == 'global_threshold':
                    ...
                    message = '{name}: Unknown frame_binarization type [{type}].'.format(name=self.__class__.__name__, type=self.params.get_path('frame_binarization.type'))
                    self.logger.exception(message)
                    raise AssertionError(message)
            else:
            ...
            # select only one event with higher probability
            processed_event_segments = []
            if len(event_segments) > 1:
                event_pro_1 = numpy.mean(frame_probabilities[event_id, event_segments[0][0]:event_segments[0][1]])
                event_pro_2 = numpy.mean(frame_probabilities[event_id, event_segments[1][0]:event_segments[1][1]])

                if event_pro_1 > event_pro_2:
                    processed_event_segments.append(event_segments[0]*self.hop_length_seconds)
                else:
                    processed_event_segments.append(event_segments[1]*self.hop_length_seconds)
            else:
                processed_event_segments = event_segments * self.hop_length_seconds
    
            # Store events
            for event in processed_event_segments:
                results.append(
                    MetaDataItem(
                        {
                            'event_onset': event[0],
