...
class TUTRareSoundEvents_2017_DevelopmentSet(SyntheticSoundEventDataset):
        ...
        if fold not in self.crossvalidation_data_train:
            ...
            for event_label_ in self.event_labels:
                ...
                if fold == 1:
                    ...
                    mixture_meta_path = os.path.join(self.local_path, 'generated_data', 'mixtures_devtrain_' + params_hash, 'meta')
                    event_list_filename = os.path.join(mixture_meta_path, 'event_list_devtrain_' + event_label_ + '.csv')
                    self.crossvalidation_data_train[fold][event_label_] = MetaDataContainer(
                    ...
                    # Load train files
                    self.crossvalidation_data_train[0][event_label_] = MetaDataContainer(filename=event_list_filename).load()
                    params_hash = self.synth_parameters.get_hash_for_path('test')
                    mixture_meta_path = os.path.join(self.local_path, 'generated_data', 'mixtures_devtest_' + params_hash, 'meta')
        ...
        if fold not in self.crossvalidation_data_test:
            ...
            for event_label_ in self.event_labels:
                ...
                if fold == 1:
                    ...
                    mixture_meta_path = os.path.join(self.local_path, 'generated_data', 'mixtures_devtest_' + params_hash, 'meta')
                    event_list_filename = os.path.join(mixture_meta_path, 'event_list_devtest_' + event_label_ + '.csv')
                    self.crossvalidation_data_test[fold][event_label_] = MetaDataContainer(filename=event_list_filename).load()
                elif fold == 0:
                    params_hash = self.synth_parameters.get_hash_for_path('train')
                    mixture_meta_path = os.path.join(self.local_path, 'generated_data', 'mixtures_devtrain_' + params_hash, 'meta')
                    event_list_filename = os.path.join(mixture_meta_path, 'event_list_devtrain_' + event_label_ + '.csv')
                    ...
                    # Load train files
                    self.crossvalidation_data_test[0][event_label_] = MetaDataContainer(filename=event_list_filename).load()
    ...
    def synthesize(self):
        ...
        for subset_label in set_progress:
            if self.log_system_progress:
                self.logger.info('  {title:<15s} [{subset_label:<30s}]'.format(title='Set ', subset_label=subset_label))
            ...
            event_meta = ParameterFile().load(filename=os.path.join(cv_setup_path, 'events_' + subset_name_on_disk + '.yaml'))
    ...
    def _generate_mixture_recipes(self, params, subset, class_label, mixture_recipes_filename, background_meta,
        ...
        for mixture_id, (bg, event_presence_flag, event_start_in_mixture_seconds, ebr, event_instance_id) in tqdm(
            ...
            if event_presence_flag:
                ...
                bg_audio, fs_bg = AudioFile(fs=params.get('mixture').get('fs')).load(filename=os.path.join(background_audio_path, bg['filepath']))

                event_audio, fs_event = AudioFile(fs=params.get('mixture').get('fs')).load(filename=os.path.join(event_audio_path, events[int(event_instance_id)]['audio_filepath']))
                ...
                if eventful_part_of_bg.shape[0] == 0:
                    message = '{name}: Background segment having an event has zero length.'.format(name=self.__class__.__name__)
                    self.logger.exception(message)
...
class TUTRareSoundEvents_2017_EvaluationSet(SyntheticSoundEventDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # if we generate challenge data by ourselves, we can set this flag to True since we already have groundtruth
        self.reference_data_present = True
        # Meta filename depends on synth parameters
        meta_filename = 'event_lists_merged.csv'
        kwargs['meta_filename'] = os.path.join('data', meta_filename)
    ...
    def _after_extract(self, to_return=None):
        ...
        if not self.meta_container.exists() and self.reference_data_present:
            message = '{name}: Meta file not found [{filename}]'.format(name=self.__class__.__name__, filename=self.meta_container.filename)
            self.logger.exception(message)
            raise IOError(message)
        ...
        if fold not in self.crossvalidation_data_train:
            ...
            for scene_label_ in self.scene_labels:
                ...
                if fold > 0:
                    ...
                    self.crossvalidation_data_train[0][scene_label_] = self.meta_container.filter(scene_label=scene_label_)
    ...
    def test(self, fold=0, scene_label=None):
        if fold not in self.crossvalidation_data_test:
            ...
            for scene_label_ in self.scene_labels:
                ...
                if fold > 0:
                    self.crossvalidation_data_test[fold][scene_label_] = MetaDataContainer(
                        filename=self._get_evaluation_setup_filename(setup_part='test', fold=fold, scene_label=scene_label_)).load()
                else:
                    self.crossvalidation_data_test[fold][scene_label_] = MetaDataContainer(
                        filename=self._get_evaluation_setup_filename(setup_part='test', fold=fold, scene_label=scene_label_)).load()
