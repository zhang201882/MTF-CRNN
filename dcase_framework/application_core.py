...
class AppCore(object):
    ...
    def _get_dataset(self, dataset=None):
        ...
        if not dataset:
            ...
            return self.Datasets[dataset](data_path=self.params.get_path('path.data'), **self.params.get_path('dataset.parameters'))
        elif isinstance(dataset, Dataset):
            return dataset
        else:
            message = '{name}: No valid dataset given [{dataset}]'.format(name=self.__class__.__name__, dataset=dataset)
            self.logger.exception(message)
            raise ValueError(message)
    ...
    def _get_learner(self, *args, **kwargs):
        ...
        if method in self.Learners:
            ...
            message = '{name}: No valid learner method given [{method}]'.format(name=self.__class__.__name__, method=method)
...
class BinarySoundEventAppCore(SoundEventAppCore):
    ...
    def feature_extraction(self, files=None, overwrite=None):
        ...
        for file_id, audio_filename in enumerate(tqdm(files,
            ...
            # Get feature filename
            current_feature_files = self._get_feature_filename(audio_file=os.path.split(audio_filename)[1], path=self.params.get_path('path.feature_extractor'))
    ...
    def feature_normalization(self, overwrite=None):
        ...
        if (self.params.get_path('feature_normalizer.enable', False) and self.params.get_path('feature_normalizer.method', 'global') == 'global'):
            if self.params.get_path('feature_normalizer.event_handling') == 'event-dependent':
                ...
                for fold in fold_progress:
                    if self.log_system_progress:
                        self.logger.info('  {title:<15s} [{fold:d}/{total:d}]'.format(title='Fold', fold=fold, total=len(fold_progress)))

                    event_labels = self.dataset.event_labels
                    # Select only active events
                    if self.params.get_path('feature_normalizer.active_events'):
                        event_labels = list(set(event_labels).intersection(self.params.get_path('feature_normalizer.active_events')))

                    for event_label in event_labels:
                        current_normalizer_files = self._get_feature_normalizer_filename(fold=fold, path=self.params.get_path('path.feature_normalizer'), event_label=event_label)
                        ...
                        for method in method_progress:
                            ...
                            if not os.path.isfile(current_normalizer_file) or overwrite:
                                ...
                                for item_id, audio_filename in enumerate(item_progress):
                                    feature_filename = self._get_feature_filename(audio_file=audio_filename, path=self.params.get_path('path.feature_extractor', {})[method])
                                    ...
                                    if os.path.isfile(feature_filename):
                                        ...
                                        message = '{name}: Features not found [{file}]'.format(name=self.__class__.__name__, file=audio_filename)
                ...
                message = '{name}: Event handling mode not implemented yet [{mode}]'.format(name=self.__class__.__name__, mode=self.params.get_path('feature_normalizer.event_handling'))
                self.logger.exception(message)
                raise ValueError(message)

            else:
                message = '{name}: Unknown event handling mode [{mode}]'.format(name=self.__class__.__name__, mode=self.params.get_path('feature_normalizer.event_handling'))
                self.logger.exception(message)
                raise ValueError(message)
        ...
        if self.params.get_path('learner.event_handling') == 'event-dependent':
            ...
            for fold in fold_progress:
                ...
                for event_label in event_progress:
                    current_model_file = self._get_model_filename(fold=fold, path=self.params.get_path('path.learner'), event_label=event_label)
                    ...
                    if not os.path.isfile(current_model_file) or overwrite:
                        ...
                        if self.params.get_path('feature_normalizer.enable'):
                            ...
                            for method, feature_normalizer_filename in iteritems(feature_normalizer_filenames):
                                if os.path.isfile(feature_normalizer_filename):
                                    ...
                                    message = '{name}: Feature normalizer not found [{file}]'.format(name=self.__class__.__name__, file=feature_normalizer_filename)
                        ...
                        # Data processor
                        data_processor = self.DataProcessor(feature_processing_chain=feature_processing_chain, data_processing_chain=data_processing_chain,)
                    ...
                    if self.params.get_path('learner.show_model_information'):
                        # Load class model container
                        model_filename = self._get_model_filename(fold=fold, path=self.params.get_path('path.learner'), event_label=event_label)

                        if os.path.isfile(model_filename):
                            model_container = self._get_learner(method=self.params.get_path('learner.method')).load(filename=model_filename)
                        else:
                            message = '{name}: Model file not found [{file}]'.format(name=self.__class__.__name__, file=model_filename)
                            self.logger.exception(message)
                            raise IOError(message)
            ...
            for fold in fold_progress:
                ...
                for event_label in event_progress:
                    if not single_file_per_fold:
                        current_result_file = self._get_result_filename(fold=fold, path=self.params.get_path('path.recognizer'), event_label=event_label)
                        if not os.path.isfile(current_result_file) or overwrite:
                            results = MetaDataContainer(filename=current_result_file)
                    ...
                    if not os.path.isfile(current_result_file) or overwrite:
                        ...
                        for item_id, item in enumerate(item_progress):
                            ...
                            # Load features
                            feature_filenames = self._get_feature_filename(audio_file=item['file'], path=self.params.get_path('path.feature_extractor'))
                            ...
                            for method, feature_filename in iteritems(feature_filenames):
                                if os.path.isfile(feature_filename):
                                    ...
                                    message = '{name}: Features not found [{file}]'.format(name=self.__class__.__name__, file=item['file'])
                                    self.logger.exception(message)
                                    raise IOError(message)
                            ...
                            if hasattr(model_container, 'data_processor'):
                                ...
                                # Backward compatibility mode
                                feature_data = model_container.feature_stacker.process(feature_data=feature_list)
                            ...
                            # Frame probabilities, with DNN model
                            frame_probabilities = model_container.predict(feature_data=feature_data,)

####################        
                            '''
                            if isinstance(feature_data, FeatureContainer):
                                tosave = feature_data.feat[0]
                            all_mel.append(tosave)
                            all_pro.append(frame_probabilities)
                            '''
####################
...
####################           
                        '''
                        np.save('/home/dingwh/results/'+target_event+'_mel.npy', all_mel)
                        np.save('/home/dingwh/results/'+target_event+'_pro.npy', all_pro)
                        '''
####################
                ...
                for event_id, event_label in enumerate(event_labels):
                    ...
                    for fold in self._get_active_folds():
                        if single_file_per_fold:
                            ...
                            result_filename = self._get_result_filename(fold=fold, path=self.params.get_path('path.recognizer'))
                        ...
                        for file_id, audio_filename in enumerate(self.dataset.test(fold, event_label=event_label).file_list):
                            ...
                            for result_item in results.filter(filename=posix_path(self.dataset.absolute_to_relative(audio_filename))):
                                if 'event_label' in result_item and result_item.event_label:
                                    current_file_results.append(result_item)

                            meta = []
                            for meta_item in self.dataset.file_meta(filename=posix_path(self.dataset.absolute_to_relative(audio_filename))):
                                if 'event_label' in meta_item and meta_item.event_label:
                                    meta.append(meta_item)

                            if meta == [] and current_file_results == []:
                                predict_onset.append(-1)
                                predict_offset.append(-1)
                                meta_onset.append(-1)
                                meta_offset.append(-1)

                            if meta == [] and current_file_results != []:
                                false_alarm_num += 1
                                predict_onset.append(current_file_results[0]['event_onset'])
                                predict_offset.append(current_file_results[0]['event_offset'])
                                meta_onset.append(-1)
                                meta_offset.append(-1)
                            ...
                            if meta != [] and current_file_results == []:
                                ...
                                predict_onset.append(-1)
                                predict_offset.append(-1)
                                meta_onset.append(meta[0]['event_onset'])
                                meta_offset.append(meta[0]['event_offset'])
                            ...
                            segment_based_metric.evaluate(reference_event_list=meta, estimated_event_list=current_file_results)
                            event_based_metric.evaluate(reference_event_list=meta, estimated_event_list=current_file_results)

                        # print missing and false alarm rate
                        self.logger.debug('Event label: {}'.format(event_label))
                        self.logger.debug('Missing: {}'.format(missing_num))
                        self.logger.debug('False alarm: {}'.format(false_alarm_num))
                        self.logger.debug('-------------------')

                        '''
                        np.save('/home/dingwh/results/'+target_event+'_pre_onset.npy', predict_onset)
                        np.save('/home/dingwh/results/'+target_event+'_pre_offset.npy', predict_offset)
                        np.save('/home/dingwh/results/'+target_event+'_meta_onset.npy', meta_onset)
                        np.save('/home/dingwh/results/'+target_event+'_meta_offset.npy', meta_offset)
                        '''
                    ...
                    if self.params.get_path('evaluator.show_details', False):
                        output += "  Event [{event}], Evaluation over {folds:d} folds\n".format(event=event_label, folds=self.dataset.fold_count)
                        output += " \n"
